package id.gits.app.sqlitemultiple.helper;
...
public class DatabaseHelper extends SQLiteOpenHelper{
    ...
    //method on create untuk mengeksekusi per table
    @Override
    ...
    //method untuk upgrade data ter table
    ...
    public Integer[] getAllIdSpesifikasiMobil(){
        ...
        if(c.getCount() >0)
        {
            Integer[] var = new Integer[c.getCount()];
            int i = 0;

            while (c.moveToNext())
            {
                var[i] = c.getInt(c.getColumnIndex(KEY_SPESIFIKASIMOBIL_ID));
                i++;
            }
            return var;
        }
    ...
    //Menampilkan Semua Harga Mobil ke List Array
